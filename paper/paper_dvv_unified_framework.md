...
$^\dagger$ AI tools were used as a research and writing assistant under the author's direction. See *Statement of AI Use* and `docs/ai_documentation/` for full transparency.

---

## Abstract

Ambient-noise monitoring of relative seismic velocity changes ($\delta v/v$) probes the subsurface mechanical state across environmental, tectonic, and volcanic settings, but interpretation remains fragmented — thermoelastic, hydrological, poroelastic, and tectonic models are applied in isolation. Here we develop a unified framework that treats $\delta v/v$ simultaneously as a stress meter and a strain meter — grounded in nonlinear (third-order) elasticity, poroelastic theory, and the induced-stress formulation of Tromp and Trampert (2018) — and show that the dominant fracture fabric selects which strain component $\delta v/v$ tracks. A bridge relation links the laboratory acoustoelastic parameter $\beta$ to the seismological pressure derivative $\mu'$, unifying the stress and strain pictures under isotropic loading and diverging diagnostically under deviatoric loading. Using only published $\delta v/v$, velocity, and geodetic data, we illustrate the framework at three contrasting sites: at Parkfield (strike-slip) the isotropic relation $\delta v/v = \beta\epsilon_{kk}$ fails — velocity tracks contractional axial strain, not dilatation — giving ~12 kPa/yr at ~0.8 km depth in fractured granite; at Cascadia (subduction) it succeeds, giving 0.58 kPa/yr in marine sediments from a borehole-calibrated $\beta$; and at Kīlauea (caldera collapse) ring fractures make $\delta v/v$ track radial strain, recovering a reservoir-stress change within a factor of two of geodesy. Because $\delta v/v$ is both frequency- and lapse-time-dependent, combining multi-frequency $\delta v/v$ with GNSS/InSAR strain could enable depth-resolved stress and strain imaging, given appropriate kernels and constraints. We map the framework's validity ranges and propose a reproducible coda-window workflow toward this goal.
...
- A unified nonlinear-elastic framework treats ambient-noise velocity change $\delta v/v$ as simultaneously a stress meter and a strain meter.
- The dominant fracture fabric sets which strain component $\delta v/v$ tracks, predicting when the scalar isotropic relation succeeds or fails.
- Applied to Parkfield, Cascadia, and Kīlauea, the framework recovers stress at depth consistent with geodetic and borehole constraints.
...
This paper addresses these gaps with a single contribution: a unified, nonlinear-elastic framework that treats $\delta v/v$ as both a stress meter and a strain meter, in which the dominant fracture fabric determines which strain component the velocity change tracks — and therefore *when* the scalar relation $\delta v/v = \beta\epsilon_{kk}$ applies and when the deviatoric formulation is required. Figure 1 summarizes the workflow, from forcing and sensitivity kernels to mechanism diagnosis and the future `codameter` implementation package. We present the theory (Section 2), quantitative scenario models for thermoelastic (Section 3), hydrological including partially saturated (Section 4), nonlinear elastic (Section 5), and anisotropic (Section 6) effects. We then develop the rheological inversion framework with emphasis on depth-resolved imaging and reproducible coda-window selection (Section 7), map validity ranges (Section 8), apply the framework quantitatively to three contrasting sites — Parkfield (strike-slip), Cascadia (subduction), and Kīlauea (volcanic) — predicting stress at depth from published $\delta v/v$ measurements (Section 9), and discuss implications (Section 10).
...
*Figures 1, 2, 4, 5, and 6 are schematic/illustrative, using synthetic $\delta v/v$ with physically realistic shapes to convey the framework's structure and diagnostics. Figures 3 and 7 and Table 2 report the quantitative three-site results derived from published measurements (§9); all numerical inputs are traced to their sources in `docs/site_analyses/provenance_tables.md`.*
...
where $\beta$ ranges from $\sim$−10 (steel) to $\sim$−10$^4$ (sandstone), reflecting microstructural compliance (Hughes and Kelly, 1953; Johnson and Rasolofosaon, 1996; Clements and Denolle, 2023).
...
Crucially, *which* modulus applies is not an assumption but a property of the data: it is set by the Péclet number $\mathrm{Pe} = \omega L^2/c$ comparing the forcing angular frequency $\omega$ to the drainage rate $c/L^2$ at the coda sensitivity depth $L = V_S/(3f)$, for hydraulic diffusivity $c$. $\mathrm{Pe} \ll 1$ is drained (use $\kappa$), $\mathrm{Pe} \gg 1$ is undrained (use $\kappa_u$), and $\mathrm{Pe}\sim 1$ is transitional. Because the seismic-band moduli inferred from $V_P, V_S$ are themselves the *undrained* moduli (the pore fluid cannot drain within a $\sim$1 Hz wave period), the natural workflow is to take $\kappa_u$ from velocities and, where the signal is drained, convert via $\kappa = \kappa_u(1-\alpha_B B)$. We apply this test explicitly at each site in Section 9; for example, the Cascadia secular trend has $\mathrm{Pe}\sim 2.5$ (transitional-to-undrained), so $\kappa_u$ from the seismic velocities is the appropriate modulus there.
...
The thermoelastic sensitivity $\partial(\rho v^2)/\partial\sigma_c$ is not an independent material parameter. For shear waves $\rho v_S^2 = \mu$, so

$$\frac{\partial(\rho v_S^2)}{\partial\sigma_c} = \frac{\partial\mu}{\partial\sigma_c} = \mu' \tag{9b}$$

neglecting the comparatively small density-change term ($\sim$2–3 orders smaller; §2.6). The thermoelastic stress sensitivity is therefore the *same* shear-modulus pressure derivative $\mu'$ that appears in the induced-stress relation (Eq. 4) and, through the bridge relation (Eq. 7), in the acoustoelastic parameter $\beta = -\mu'\kappa/(2\mu)$. A single depth profile $\mu'(z)$ thus governs the thermoelastic, hydrological, and tectonic limbs of the framework — which is why the published ranges for $\partial(\rho v^2)/\partial\sigma_c$ ($\sim$50–1000; §3.3) and for $\mu'$ (Fokker et al., 2021) coincide. This identity is the quantitative core of the "unified" framework and is the basis for the consistency expectation in §10.1.
...
1. **Dynamic capillarity is essential.** Models with static capillary stress underestimate velocity increases during evaporation by $\sim$50%, while models without any capillary stress produce significantly smaller velocity variations in both wetting and drying.
...
$$\Delta\left(\frac{\delta V_S}{V_S}\right)_{\text{vert. vs. horiz.}} = \frac{T_{33}^0}{2\mu} \tag{11b}$$
...
encodes the distribution of relaxation timescales. At Parkfield, $\tau_{\max}$ is constrained to 1–30,000 years, meaning post-2004 healing may continue today (Figure 4; detailed healing curves in Figure S10). The trade-off between healing and tectonic trend is a fundamental limitation requiring independent geodetic constraints.

### 7.2 Distinguishing Rheological Models

Different rheologies produce distinct $\delta v/v$–strain signatures (Figure 4): elastic (linear crossplot), Maxwell (exponential decay), Kelvin-Voigt (delayed buildup), SLS (frequency-dependent dispersion, elliptical crossplot), and slow dynamics (logarithmic recovery, hysteresis). Tidal modulation (Sens-Schönfelder and Eulenfeld, 2019) provides the cleanest diagnostic at well-known forcing periods.
...
where $K(z, f)$ is the Rayleigh or Love wave phase-velocity sensitivity kernel at frequency $f$ (Figure 6a). The peak sensitivity depth is approximately $V_s / (3f)$, ranging from $\sim$50 m at 2 Hz to $\sim$5 km at 0.1 Hz for typical crustal velocities.

![Figure 6](../figures/main/fig06_depth_kernels.png)

**Figure 6.** Frequency-dependent depth sensitivity and inversion limits. (a) Schematic Rayleigh/Love sensitivity kernels showing broad, overlapping depth sensitivity for representative frequency bands. (b) Peak sensitivity depth $z_{\mathrm{peak}} \approx V_S/(3f)$ for soft sediment, weathered rock, and crystalline rock. (c) Relative uncertainty reduction for independent versus correlated frequency bands.
...
The homogeneous half-space assumption fails when velocity contrasts exceed $\sim$2:1 within the sensitivity kernel (Figure 6; detailed validity tests in Figure S11). At soft sedimentary sites — such as Mexico City, where $V_S$ ranges from 50 m/s in the lake zone to 800 m/s in bedrock — the homogeneous assumption breaks down at all frequencies, requiring station-specific velocity profiles and proper sensitivity kernels (Ermert et al., 2023). At hard-rock stations above 1 Hz, sensitivity is confined to the upper few meters where the medium is more likely to be approximately uniform.

### 8.2 Linear Acoustoelasticity

The linear acoustoelastic approximation is valid for strains below $\sim$10$^{-5}$ (Figure S12), covering tidal, thermoelastic, and most hydrological signals, but breaks down for coseismic and strong-motion strains. The regime diagram (Figure 1; detailed version in Figure S7) maps dominant processes versus frequency and depth, including the capillary/vadose zone regime at high frequencies.
...
| $\mathrm{Pe}=\omega L^2/c$ | $\ll1$ to $\gg1$ | Selects drained ($\kappa$) vs undrained ($\kappa_u$) modulus in Eq. 7 | Data-driven per signal; needs $c$ and $L$ |
...
This framework yields three transferable results (summarized in Table 2). First, the same shear-modulus pressure derivative $\mu'(z)$ underlies the thermoelastic, hydrological, and tectonic responses, so a single material profile — not three independent sensitivities — governs $\delta v/v$ at a site. Second, whether the scalar relation $\delta v/v = \beta\epsilon_{kk}$ applies is set by the loading geometry and fracture fabric: it succeeds for predominantly volumetric loading (Cascadia) and fails under deviatoric loading with oriented cracks (Parkfield, Kīlauea), where the controlling strain component must be identified. Third, because $\delta v/v$ amplitudes scale with $\beta$, raw amplitudes cannot be compared across sites without normalizing by the material sensitivity (§9.4). We now discuss the unifying mechanism, the alternatives these results must be weighed against, monitoring implications, a practical workflow, and limitations.

### 10.1 The Unifying Power of Nonlinear Elasticity

All $\delta v/v$ sources — thermoelastic, hydrological, tectonic, volcanic — operate through the same mechanism: stress-dependent modification of elastic moduli via compliant porosity and microcracks. This is not merely a conceptual unity: the three sensitivity parameters reduce to one. For shear waves the thermoelastic sensitivity equals the induced-stress pressure derivative, $\partial(\rho v^2)/\partial\sigma_c = \mu'$ (Eq. 9b), and the acoustoelastic parameter follows from the bridge relation, $\beta = -\mu'\kappa/(2\mu)$ (Eq. 7). A single material property — the shear-modulus pressure derivative $\mu'(z)$ — therefore governs all forcings at a given site, so the *same* $\mu'(z)$ must consistently explain the thermoelastic, hydrological, and tectonic responses. Inconsistencies diagnose either unmodeled forcings or violations of the isotropic assumption (under deviatoric loading the scalar $\beta$ no longer suffices; §6).

These site-by-site successes should be weighed against alternative explanations rather than read as validation. The quantitative agreements in §9 — within a factor of 1.4 at Parkfield and a factor of 2 at Kīlauea — are consistency checks computed with a bridge relation that, applied to a single loading direction, is only order-of-magnitude (Eq. 7 was derived for isotropic loading; §2.5); they constrain the mechanism but do not uniquely confirm it. Competing interpretations remain viable. At Parkfield, the secular $\delta v/v$ increase may partly reflect the long tail of post-2004 logarithmic healing rather than purely inter-seismic loading — the healing–trend trade-off is intrinsic and underdetermined without independent constraints (§7.1; Okubo et al., 2024). At Cascadia, progressive sediment consolidation or dewatering of the accretionary wedge could contribute to the trend alongside elastic strain accumulation, a mechanism the elastic framework does not separate. More generally, apparent $\delta v/v$ from seasonal changes in the noise-source field (Zhan et al., 2013) can masquerade as a secular signal if unmitigated. Discriminating among these requires the independent geodetic, multi-frequency, and multi-component constraints emphasized throughout: the framework sharpens and quantifies the competing hypotheses but, on published data alone, does not exclude them.
...
For **hydrological monitoring**, the depth-resolved approach (Section 7.3) can separate shallow capillary effects from deep pore-pressure changes, and the competition between loading and pore-pressure effects can in principle be resolved rather than assumed, given adequate multi-frequency coverage and independent constraints (no worked example is presented here).
...
7. At Parkfield, the framework predicts a deviatoric stress rate of ~12 kPa/yr at ~0.8 km depth with $\beta_{\text{axial}} \approx 240$ and $\mu' \approx 250$, cross-checked against GNSS strain (agreement to a factor of 1.4). At Cascadia, the framework reproduces an isotropic stress rate of 0.58 kPa/yr at ~0.2 km depth with $\mu' \approx 620$ — but here $\beta \approx 3160$ is itself calibrated from the borehole strain, so this is an internal consistency check, not an independent validation. At Kīlauea, a co-collapse radial stress of ~170 kPa is recovered from $\delta v/v \approx 0.5\%$ with $\beta_{\text{radial}} \approx 300$ and $\mu' \approx 360$, consistent with the geodetically constrained reservoir pressure change within a factor of 2. The directional $\beta$ (and hence $\mu'$) at Parkfield and Kīlauea follow from the bridge relation applied to a single loading direction and are order-of-magnitude estimates (Eq. 7 was derived for isotropic loading). These results use only published measurements; end-to-end validation on independently reprocessed data is future work (§10.5). The effective $|\beta|$ for fractured crystalline rock (granite, basalt) clusters around 200–400, while unconsolidated marine sediment gives $|\beta| \sim 3000$.
